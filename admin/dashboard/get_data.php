...
try {
    if ($month === 'overall') {
        $sql = "SELECT 
                ...
                LEFT JOIN orderpos o ON months.month = MONTH(o.orderDate) AND YEAR(o.orderDate) = ?
                LEFT JOIN (
                    SELECT MONTH(ORDEREDDATE) as month, SUM(PAYMENT) as PAYMENT
                    FROM tblsummary
                    WHERE YEAR(ORDEREDDATE) = ?
        ...
        $sql = "SELECT 
                ...
                LEFT JOIN (
                    SELECT MONTH(ORDEREDDATE) as month, SUM(PAYMENT) as PAYMENT
                    FROM tblsummary
                    WHERE YEAR(ORDEREDDATE) = ? AND MONTH(ORDEREDDATE) = ?
                    GROUP BY MONTH(ORDEREDDATE)
                ) s ON months.month = s.month";
        
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, 'iiiii', $month, $month, $year, $year, $month);
    }
